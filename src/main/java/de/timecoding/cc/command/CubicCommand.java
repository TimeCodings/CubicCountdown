        ...
        if (commandSender.isOp()) {
            if (strings.length == 1) {
                ...
                if (plugin.getConfigHandler().keyExists(strings[1])) {
                    if (isBoolean(entry)) {
                        plugin.getConfigHandler().getConfig().set(strings[1], Boolean.parseBoolean(entry));
                    } else if (isInteger(entry)) {
                        plugin.getConfigHandler().getConfig().set(strings[1], Integer.parseInt(entry));
                    } else if (isStringList(entry)) {
                        plugin.getConfigHandler().getConfig().set(strings[1], toStringList(entry));
                    } else {
                        plugin.getConfigHandler().getConfig().set(strings[1], entry);
                    }
                    plugin.getConfigHandler().save();
                    plugin.getConfigHandler().reload();
                    plugin.getCubicAPI().startActionbar();
                    commandSender.sendMessage("§aSuccessfully set §e" + strings[1] + "'s §avalue to §e" + entry + "§a! §7(config.yml)");
                } else if (plugin.getDataHandler().keyExists(strings[1])) {
                    if (isBoolean(entry)) {
                        plugin.getDataHandler().getConfig().set(strings[1], Boolean.parseBoolean(entry));
                    } else if (isInteger(entry)) {
                        plugin.getDataHandler().getConfig().set(strings[1], Integer.parseInt(entry));
                    } else if (isStringList(entry)) {
                        plugin.getDataHandler().getConfig().set(strings[1], toStringList(entry));
                    } else {
                        plugin.getDataHandler().getConfig().set(strings[1], entry);
                    }
                    plugin.getDataHandler().save();
                    plugin.getDataHandler().reload();
                    plugin.getCubicAPI().startActionbar();
                    commandSender.sendMessage("§aSuccessfully set §e" + strings[1] + "'s §avalue to §e" + entry + "§a! §7(datas.yml)");
                } else {
                    commandSender.sendMessage("§cThe key §e" + strings[1] + " §cdoes not exist!");
                }
            } else if (strings.length == 3 && strings[0].equalsIgnoreCase("simulate")) {
                String map = strings[2].toUpperCase();
                if (plugin.getCubicAPI().cubeNameExists(map)) {
                    Cube cube = plugin.getCubicAPI().getCubeByName(map);
                    if (strings[1].equalsIgnoreCase("win")) {
                        plugin.getCubicAPI().increaseWins(cube);
                        commandSender.sendMessage("§aA §ewin §awas successfully simulated!");
                    } else if (strings[1].equalsIgnoreCase("lose")) {
                        plugin.getCubicAPI().increaseLoses(cube);
                        commandSender.sendMessage("§aA §close §awas successfully simulated!");
                    } else if (strings[1].equalsIgnoreCase("help")) {
                        plugin.getCubicAPI().increaseHelpCounter(cube);
                        commandSender.sendMessage("§aA §fhelp §awas successfully simulated!");
                    } else {
                        commandSender.sendMessage("§cThe variable §e" + strings[1] + " §cdoes not exist!");
                    }
                } else {
                    commandSender.sendMessage("§cThe map §e" + map + " §cdoes not exist! You can create it with §e/cc setup");
                }
